...
class IdentityConfigFlow(ANSSubEntryFlowBase):
    ...
    async def _store_entry(self) -> SubentryFlowResult:
        ...
        try:
            ...
            identity_obj = RecipientData(
                id=identity_id,
                type=RecipientType(id_type),
                name=name,
                email=email,
                phone=phone,
            )
            ...
            identity_config_obj = RecipientConfig.from_dict(self._identity_settings)
    ...
    async def async_step_identity_selection(
        ...
        try:
            from .helper import get_not_configured_ha_users  # noqa: PLC0415
        ...
        if user_input is not None:
            try:
                ...
                if identity_type and identity_type == RecipientType.HA_USER.value:
    ...
    async def async_step_identity_definition(
        ...
        if user_input is not None:
            try:
                ...
                if (
                    not await async_check_recipient_name_availability(
