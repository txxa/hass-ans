...
- **Acknowledgement tracking** — ANS tracks whether a delivered notification was acknowledged: tapping an action button or the notification body on a mobile push, or dismissing a persistent notification in the HA sidebar, fires an `ans_notification_acknowledged` HA bus event (with `method`, `device_name`, and `user_id` in the event context) and records acknowledgement state in a persistent `AcknowledgementRegistry`; automations can correlate the `notification_id` returned by `ans.send_notification` with this event to detect confirmed read receipts, including after HA restarts
